...
uniform sampler2DRect snowSampler; //Snow Support
...
void addWaterColor(in vec2 fragCoord,inout vec4 baseColor)
	...
	vec2 alpha = texture2DRect(snowSampler, waterTexCoord).rg; //Snow Support
	
	/* Check if the surface has snow: */	
	if( alpha.r >0.000001) //Test Line

		{
		/* Calculate the water color: */
		
		vec3 wn=normalize(vec3((texture2DRect(bathymetrySampler,vec2(waterTexCoord.x-1.0,waterTexCoord.y)).r-
		                        texture2DRect(bathymetrySampler,vec2(waterTexCoord.x+1.0,waterTexCoord.y)).r)*waterCellSize.y,
		                       (texture2DRect(bathymetrySampler,vec2(waterTexCoord.x,waterTexCoord.y-1.0)).r-
		                        texture2DRect(bathymetrySampler,vec2(waterTexCoord.x,waterTexCoord.y+1.0)).r)*waterCellSize.x,
		                       2.0*waterCellSize.x*waterCellSize.y)); 
		float colorW=pow(dot(wn,normalize(vec3(0.0529,0.0702,0.75))),100.0)*1.0-0.0; //Test Line
		...
		vec4 waterColor=vec4(1.0,1.0,1-colorW,1.0); // test line
		
		//vec4 waterColor=vec4(1-colorW,1.0,1.0,1.0);

// Vertical Stripes

//		if(b < 0.0){ 								//Bottom Stripe
//			waterColor=vec4(1.0,1-colorW,0.0,1.0);
//		}else if(b < 4.0 && b > 0.1){				 //Middle Stripe
//			waterColor=vec4(colorW,colorW,1.0,1.0); 
//		}else{ 										//Top Stripe
//			waterColor=vec4(1-colorW,1.0,1.0,1.0);
//			}

		/* Mix the water color with the base surface color based on the water level: */
		baseColor=mix(baseColor,waterColor,min((alpha.r)*waterOpacity,1.0));
		}
	...
	if(waterLevel>0.000001)
		{
			...
			vec3 wn=normalize(vec3((texture2DRect(quantitySampler,vec2(waterTexCoord.x-1.0,waterTexCoord.y)).r-
			                       ...
			                       2.0*waterCellSize.x*waterCellSize.y));
			float colorW=pow(dot(wn,normalize(vec3(0.075,0.075,1.0))),100.0)*1.0-0.0; //og line

			vec4 waterColor=vec4(colorW,colorW,1.0,1.0); // Water
			// vec4 waterColor=vec4(1.0-colorW,1.0-colorW*2.0,0.0,1.0); // Lava
			// vec4 waterColor=vec4(0.0,0.0,1.0,1.0); // Blue
			//vec4 waterColor=vec4(0.176,0.239,0.192,1.0); // test line
...
//			if(gl_FragCoord.x > horzLine && gl_FragCoord.y > vertLine){ //Top Right
//				waterColor=vec4(.0952,1-colorW,0.953,1.0);
//			}else if(gl_FragCoord.x > horzLine && gl_FragCoord.y < vertLine){ //Bottom Right
//				waterColor=vec4(colorW,colorW,1.0,1.0); 
//			}else if(gl_FragCoord.x < horzLine && gl_FragCoord.y >vertLine){ //Top Left
//				waterColor=vec4(1.0-colorW,1.0-colorW*2.0,0.0,1.0);
//			}else{ 																//Bottom Left
//				waterColor=vec4(1-colorW,1.0,1.0,1.0);
//			}
