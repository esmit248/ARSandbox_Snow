...
void addWaterColor(in vec2 fragCoord,inout vec4 baseColor)
	...
	vec2 snow = texture2DRect(snowSampler, waterTexCoord).rg; //Snow Support
	
	/*Check if Surface is under water:*/
	if(waterLevel>0.000001) //Snow: Changed for testing 
	{
			...
			// vec4 waterColor=vec4(0.75,0.3,1.0,1.0); // Solid Color
			// vec4 waterColor=vec4(0.0,0.0,0.0,0.0); // test line
			// vec4 waterColor=vec4(0.251,0.929,0.306,1.0); //Slime
	...
	if(snow.r>0.000001)
		...
		//vec4 waterColor=vec4(colorW,colorW,1.0,1.0); // Water
		//vec4 waterColor=vec4(1.0-colorW,1.0-colorW*2.0,0.0,1.0); // Lava
		vec4 waterColor=vec4(1-colorW,1.0,1.0,1.0);	//Snow


		// Mix the water color with the base surface color based on the water level: 
		baseColor=mix(baseColor,waterColor,min((snow.r)*waterOpacity,1.0));
		}
	}
