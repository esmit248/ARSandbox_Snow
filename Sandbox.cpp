...
void Sandbox::addWater(GLContextData& contextData) const
	...
	if(handExtractor!=0&&!handExtractor->getLockedExtractedHands().empty())
		...
		/*Spring/Constant Water Source Support*/
		/*
		Scalar testhh = Math::div2(waterTable->cellSize[1]);
		Scalar testhw = Math::div2(waterTable->cellSize[0]);
		...
		Scalar springLocationX=(waterTable->domain.min[0]+testhw)*0.75;
		Scalar springLocationY=(waterTable->domain.max[1]-testhh)*0.57;

		Point springCenter = Point(-37,18,-66);
		double springRadius = (3.0);

		for(int i=0;i<32;++i)
		{
		Scalar angle=Scalar(2)*Math::Constants<Scalar>::pi*Scalar(i)/Scalar(32);
		glVertex(testCenter+x*(Math::cos(angle)*testRadius*0.75)+y*(Math::sin(angle)*testRadius*0.75));
		}
		glEnd();
		glPopAttrib();
		*/
