	...
	try
		...
		*(ulPtr++)=glGetUniformLocationARB(result,"depthProjection");
		...
		if(waterTable!=0&&dem==0)
			...
			*(ulPtr++)=glGetUniformLocationARB(result,"snowSampler"); //Snow Support
...
void SurfaceRenderer::initContext(GLContextData& contextData) const
	...
	//dataItem->globalAmbientHeightMapShaderUniforms[10]=glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader,"snowSampler");
...
void SurfaceRenderer::renderSinglePass(const int viewport[4],const PTransform& projection,const OGTransform& modelview,GLContextData& contextData) const
	...
	if(waterTable!=0&&dem==0)
		...
		/*Snow Support: */
		//Bind the snow texture:  9/13/2021 need this to prevent glPrintError: Invalid Operation BCS
		glActiveTextureARB(GL_TEXTURE5_ARB);
		waterTable->bindSnowTexture(contextData);
		glTexParameteri(GL_TEXTURE_RECTANGLE_ARB,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
		glTexParameteri(GL_TEXTURE_RECTANGLE_ARB,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
		glTexParameteri(GL_TEXTURE_RECTANGLE_ARB,GL_TEXTURE_WRAP_S,GL_CLAMP_TO_EDGE);
		glTexParameteri(GL_TEXTURE_RECTANGLE_ARB,GL_TEXTURE_WRAP_T,GL_CLAMP_TO_EDGE);
		glUniformARB(*(ulPtr++),5);
